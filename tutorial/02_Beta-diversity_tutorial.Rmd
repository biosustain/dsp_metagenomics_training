...
- **Output**: A 2D scatter plot where:
  - Each point = a sample.
  - Distance between points = ecological dissimilarity.
...
- **k = 2:** return 2 dimensions, typically the top 2 axe
...
to the PCoA axes), you can use the envfit() function in the vegan package. Fit environmental vectors to the ordination
...
p <- ggplot(scores, 
  ...
  geom_point(aes(color = Area, 
                 fill = Area, 
                 shape = Sampling, 
                 text = paste("Area:", Area, "<br>Sampling:", Sampling)), 
             size = 2) +

  # Vector arrows as segments (arrowhead won't appear in plotly)
  geom_segment(data = loadings_df,
               aes(x = 0, 
                   y = 0, 
                   xend = Axis1, 
...
<div class="tip">
  <strong>💡 Tip:</strong> You can plot all shapes in R ggplot.
</div>

```{r}
# Create a data frame with all shape values from 0 to 25
shapes_df <- data.frame(
  shape = 0:25,
  x = rep(1:6, length.out = 26),
  y = rep(5:1, each = 6)[1:26]
)

# Plot all shapes
ggplot(shapes_df, aes(x = x, y = y)) +
  geom_point(aes(shape = factor(shape)), size = 4, fill = "blue", color = "black") +
  geom_text(aes(label = shape), vjust = -1) +
  scale_shape_manual(values = 0:25) +
  theme_void() +
  ggtitle("Available ggplot2 Shape Values (0–25)") +
...
rmarkdown::render("BetaDiversity.Rmd")
